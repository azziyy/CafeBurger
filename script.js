...
window.onload=()=>{
    ...
    if(uid)checkAuth(uid); else {document.getElementById('loginModal').style.display='flex'; document.getElementById('loaderScreen').style.display='none';}
...
function checkAuth(uid){ db.ref('users/'+uid).on('value',s=>{ if(s.val()){ user=s.val(); document.getElementById('loginModal').style.display='none'; initApp(); } else document.getElementById('loginModal').style.display='flex'; document.getElementById('loaderScreen').style.display='none'; }); }
...
function loadStories(){
    fetch(`https://docs.google.com/spreadsheets/d/${ADS_ID}/gviz/tq?tqx=out:json`).then(r=>r.text()).then(t=>{
        ...
        document.getElementById('storiesContainer').innerHTML=stories.map((u,i)=>`<div class="story-ring tap" onclick="openStory(${i})"><img src="${u}"></div>`).join('');
    });
}
...
function showStory(){
    ...
    document.getElementById('stImg').src=stories[stIdx];
    document.getElementById('storyBars').innerHTML=stories.map((_,i)=>`<div class="st-bar"><div class="st-fill ${i===stIdx?'anim':(i<stIdx?'seen':'')}"></div></div>`).join('');
...
function renderCats(){ const c=['Barchasi',...new Set(products.map(p=>p.cat))]; document.getElementById('catNav').innerHTML=c.map(k=>`<div class="cat-btn tap ${k=='Barchasi'?'active':''}" onclick="filt('${k}',this)">${k}</div>`).join(''); }
function filt(c,el){ document.querySelectorAll('.cat-btn').forEach(x=>x.classList.remove('active')); el.classList.add('active'); renderGrid(c); }
function renderGrid(c){
    ...
    document.getElementById('menuGrid').innerHTML=l.map(p=>{
        const q=cart[p.id]||0;
        const btn = q>0 ? `<div class="qty-pill tap" onclick="event.stopPropagation()"><div class="qp-btn" onclick="updCart(${p.id},-1)">-</div><b>${q}</b><div class="qp-btn" onclick="updCart(${p.id},1)">+</div></div>` : `<div class="btn-plus tap" onclick="updCart(${p.id},1);event.stopPropagation()">+</div>`;
        return `<div class="card tap" onclick="openProd(${p.id})">
            <div class="card-img-box"><img src="${p.img}"><div class="like-btn ${favs[p.id]?'active':''}" onclick="togFav(${p.id});event.stopPropagation()"><i class="fa fa-heart"></i></div></div>
            <div class="card-content"><div class="c-title">${p.name}</div><div class="c-foot"><div class="c-price">${p.price.toLocaleString()} so'm</div>${btn}</div></div>
        </div>`;
    }).join('');
}

function updCart(id,n){ cart[id]=(cart[id]||0)+n; if(cart[id]<=0)delete cart[id]; renderGrid(document.querySelector('.cat-btn.active').innerText); renderCart(); }
function renderCart(){
    let t=0, h='';
    const k=Object.keys(cart);
    document.getElementById('navBadge').innerText=k.length; document.getElementById('navBadge').classList.toggle('hidden',k.length===0);
    for(let id in cart){
        const p=products.find(x=>x.id==id); t+=p.price*cart[id];
        h+=`<div class="cart-item"><img src="${p.img}" class="c-img"><div class="c-info"><h4>${p.name}</h4><p>${p.price.toLocaleString()} so'm</p></div><div class="qty-pill"><div class="qp-btn tap" onclick="updCart(${p.id},-1)">-</div><b>${cart[id]}</b><div class="qp-btn tap" onclick="updCart(${p.id},1)">+</div></div></div>`;
    }
...
function getGPS(){
    ...
    navigator.geolocation.getCurrentPosition(p=>{
        ...
        if(shopLoc && shopLoc.radius){
            ...
            if(d>shopLoc.radius){ b.innerHTML="❌ Uzoq masofa"; b.className='btn-gps err tap'; loc=null; renderCart(); return; }
        }
        loc={lat,long:lng}; b.innerHTML="✅ Manzil aniqlandi"; b.className='btn-gps ok tap'; renderCart();
...
function sendOrder(){
    ...
    cart={}; loc=null; activePromo=null; toast("✅ Buyurtma yuborildi"); nav('history',document.querySelectorAll('.nav-item')[2]);
    document.getElementById('gpsBtn').innerHTML=`<i class="fa fa-map-marker-alt"></i> Manzilni aniqlash`; document.getElementById('gpsBtn').className='btn-gps tap';
}

function loadChat(){
    db.ref('messages/'+user.phone.replace('+','')).limitToLast(50).on('child_added', s=>{
        const m=s.val();
        if(m.from==='admin' && document.getElementById('chatModal').style.display==='none') document.getElementById('chatBadge').style.display='block';
...
function openPromoList(){
    ...
    db.ref('promocodes').once('value',s=>{
        ...
        let h=""; for(let k in d){if(d[k].limit>0) h+=`<div style="background:rgba(255,255,255,0.05);padding:10px;border-radius:10px;margin-bottom:5px;display:flex;justify-content:space-between;"><b>${k}</b><span style="color:var(--success)">${d[k].discount}%</span></div>`;}
...
function openModal(id){document.getElementById(id).style.display='flex'; if(id=='chatModal')document.getElementById('chatBadge').style.display='none';}
...
function togFav(id){if(favs[id])delete favs[id];else favs[id]=true;db.ref('users/'+user.phone.replace('+','')+'/favorites').set(favs);renderGrid(document.querySelector('.cat-btn.active').innerText);}
function openFavs(){const l=products.filter(p=>favs[p.id]);document.getElementById('favList').innerHTML=l.map(p=>`<div class="fav-card"><div class="fav-del" onclick="togFav(${p.id});openFavs()">✕</div><img src="${p.img}"><div style="font-size:11px;">${p.name}</div><button style="width:100%;margin-top:5px;background:var(--gold);border:none;border-radius:6px;" onclick="updCart(${p.id},1);closeModal('favModal')">+</button></div>`).join('')||"Bo'sh";openModal('favModal');}
